...
export async function TypingSessionPage({ mode }: { mode: AttemptMode }) {
  const { student } = await requireStudent();
  const typingContext = await ensureAttemptForStudent(student.id, mode);
  const modeLabel = getAttemptModeLabel(mode);

  if (typingContext.state === 'no-article') {
    return (
      <StudentPageShell studentName={student.name} title={`${modeLabel}模式`} description="系统暂时没有可用文章。">
        <Card>
          <CardHeader>
  ...
  if (typingContext.state === 'locked') {
    return (
      <StudentPageShell studentName={student.name} title={`${modeLabel}模式`} description="当前账号已达到正式考试尝试次数上限。">
        <Card>
          <CardHeader>
  ...
  if (typingContext.state !== 'ready' || !typingContext.attempt) {
    return (
      <StudentPageShell studentName={student.name} title={`${modeLabel}模式`} description="系统未能正确初始化当前记录。">
        <Card>
          <CardHeader>
  ...
  return (
    <StudentPageShell
      ...
      title={`${modeLabel}模式`}
      description={mode === 'practice' ? '练习成绩不会进入正式排行榜。' : '正式考试成绩会进入排行榜与后台统计。'}
      extraInfo={
